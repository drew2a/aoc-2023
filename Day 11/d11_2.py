...
def expand_the_universe(m, gals):
    ...
    result = []
    for g in gals:
        affected = len(list(i for i in not_occupied_i if i < g[0]))
        result.append((g[0] + (affected * (EXPAND_VALUE - 1)), g[1]))

    final_result = []
    for g in result:
        affected = len(list(j for j in not_occupied_j if j < g[1]))
        final_result.append((g[0], g[1] + (affected * (EXPAND_VALUE - 1))))
